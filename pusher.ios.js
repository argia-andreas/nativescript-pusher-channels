...
var TNSPusher = (function (_super) {
    ...
    TNSPusher.prototype.subscribe = function (event) {
        ...
        if (!channel) {
            channel = this.ios.subscribeWithChannelNameOnMemberAddedOnMemberRemoved(event, function (p1) {
            }, function (p1) {
            });
        }
...
var TNSPusherChannel = (function (_super) {
    ...
    TNSPusherChannel.prototype.bind = function (event, callback) {
        ...
        if (owner) {
            owner._channelEvents.set(callback, { id: id, event: event, channelName: this.channel.name });
        }
    };
        ...
        if (event === 'state_change' || event === 'connected') {
            ...
            var id_1 = this.ios.bindWithEventCallback(function (ev) {
                if (ev) {
                    ...
                    var data = reader.readProp(ev, 'data', interop.types.id);
                    var userId = reader.readProp(ev, 'userId', interop.types.id);
                    if (event === 'error' && eventName === pusher_common_1.InternalPusherEvents.Error) {
                        callback({
                            channelName: channelName,
                            data: JSON.parse(data)
                        });
                    ...
                    if (event === 'ping' && eventName === pusher_common_1.InternalPusherEvents.Ping) {
                        callback('ping');
                        owner._connectionEvents.set(callback, { event: event, id: id_1 });
                    }
                    if (event === 'pong' && eventName === pusher_common_1.InternalPusherEvents.Pong) {
                        callback('pong');
                        owner._connectionEvents.set(callback, { event: event, id: id_1 });
                    }
                }
...
var PusherDelegateImpl = (function (_super) {
    ...
    PusherDelegateImpl.prototype.changedConnectionStateFromTo = function (old, new_) {
    ...
    PusherDelegateImpl.prototype.failedToSubscribeToChannelWithNameResponseDataError = function (name, response, data, error) {
        var owner = this._owner.get();
        if (owner) {
